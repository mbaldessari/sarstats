...
def main():
    ...
    print(
        f"Parsing files: {' '.join(map(os.path.basename, args.sar_files))}",
        end="",
    )
    if len(args.sar_files) == 0:
    ...
    sar_stats = SarStats(sar_grapher, args.maxgraphs)
    ...
    if args.csv_file is not None:
        print(f"Export to csv: {args.csv_file}")
